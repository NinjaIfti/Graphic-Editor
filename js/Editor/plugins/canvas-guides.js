...
function showPositioningLine(type, point, props) {
    ...
    if (type === "X") {
        ...
        if (props.pointType == "canvas")
            x2 = canvasDimensions.resizedWidth;
        coords = [p2, 0, x2, 0];
    } else if (type === "Y") {
        let y2 = obj.top + (obj.getScaledHeight() / 2);
        if (props.pointType == "canvas")
            y2 = canvasDimensions.resizedHeight;
        coords = [0, p2, 0, y2];
    }
    if (line) {
        line.set({
    ...
    canvas.add(line);
    // alignmentObject(alignment, line);
    // canvas.renderAll();
}
...
function getAllPositioningPoints() {
    let pointsX = [],
        ...
        // pointsX = [canvasDimensions.resizedWidth / 2],
        // pointsY = [canvasDimensions.resizedHeight / 2],
    ...
    if (isObjInGroup(activeObjId)) {
        // select group
        if (activeObj.group)
            activeObjId = activeObj.group.id;
    }
