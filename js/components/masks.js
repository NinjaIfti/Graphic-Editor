  ...
  return {
      ...
      if (window.canvas) {
        ...
        window.canvas.on("selection:updated", (e) => {
          if (e.selected && e.selected[0] && e.selected[0].type === "image") {
            ...
            console.log(
              "[MASK] Selection changed to non-image, clearing cache"
            );
          }
        });
    ...
    applyMask(maskSrc) {
      ...
      console.log("[MASK] Target image dimensions:", {
        ...
        scaled: {
          width: targetObject.getScaledWidth(),
          height: targetObject.getScaledHeight(),
        },
        display: {
      ...
      console.log("[MASK] Looking for mask template with ID:", maskId);
      const maskTemplate = this.svgMaskTemplates[maskId];

      if (!maskTemplate) {
        console.error("[MASK] No SVG template found for mask ID:", maskId);
        return;
      }
      ...
      // Load the mask as an image first to ensure proper sizing
      const img = new Image();
