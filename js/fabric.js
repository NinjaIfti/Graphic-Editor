// src/fabric.js
import {Canvas, Textbox, Rect, Path, PencilBrush, Circle, Line, FabricImage, ActiveSelection, filters} from 'fabric';
...
export default function fabricComponent() {
    return {
        ...
        clearCanvas() {
            if (!window.canvas) return;
            window.canvas.clear();
            window.canvas.setBackgroundColor("#ffffff", window.canvas.renderAll.bind(window.canvas));
            this.addToHistory();
        },
        ...
        saveCanvasState() {
            if (!window.canvas) return;
            const json = window.canvas.toJSON(['lockMovementX', 'lockMovementY', 'lockRotation', 'lockScalingX', 'lockScalingY']);
        ...
        init() {
            ...
            this.$watch("activeTool", (value) => {
                // Simply update the active tool without trying to call methods on toolbar
                // This fixes the "this.toolbar.handleToolChange is not a function" error
            ...
            try {
                ...
                if (activeObject) {
                    ...
                    if (this.toolbar) {
                        ...
                        this.toolbar.isMultiSelection = activeObject.type === 'activeSelection';
                    }
        ...
        handleObjectModified(e) {
            try {
                ...
                console.error("Error handling object modified:", error);
            }
        },
        ...
        handleObjectAdded(e) {
            try {
                ...
                console.error("Error handling object added:", error);
            }
        },
        ...
        handleObjectRemoved(e) {
            try {
                ...
                console.error("Error handling object removed:", error);
            }
        },
        ...
        applyFilter(imageObject, filterType, options) {
            ...
            switch (filterType) {
                ...
                case 'invert':
                    ...
                    break;
                case 'blur':
                    imageObject.filters.push(new filters.Blur({
                        blur: options?.amount || 0.5
                    }));
                    break;
                case 'contrast':
                    imageObject.filters.push(new filters.Contrast({
                        contrast: options?.amount || 0.25
                    }));
                    break;
                case 'brightness':
                    imageObject.filters.push(new filters.Brightness({
                        brightness: options?.amount || 0.1
                    }));
                    break;
                case 'saturation':
                    imageObject.filters.push(new filters.Saturation({
                        saturation: options?.amount || 0.3
                    }));
                    break;
                case 'noise':
                    imageObject.filters.push(new filters.Noise({
                        noise: options?.amount || 100
                    }));
                    break;
                case 'pixelate':
                    imageObject.filters.push(new filters.Pixelate({
